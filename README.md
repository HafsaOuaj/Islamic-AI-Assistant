...
The generator component of the RAG pipeline is evaluated using the ROUGE-L metric. ROUGE-L is based on the longest common subsequence (LCS) between the generated output and the reference answer, providing a measure of both precision and recall while preserving word order.

The evaluation results are presented in the figure below:
![alt text](resources/doc/image_6.png)


The ROUGE-L evaluation shows that the generator is able to produce responses that partially align with the ground-truth answers, with a moderate level of lexical overlap. However, the results also highlight variability across samples, indicating that performance is not fully consistent for all queries.

Overall, while the model demonstrates a reasonable ability to capture relevant information from the retrieved context, there is still room for improvement in both answer completeness and faithfulness, particularly for challenging or ambiguous queries. Future improvements should focus on enhancing retrieval quality and reducing generation variance to achieve more stable and accurate outputs.

# Analysis & Findings

We conduct a qualitative and quantitative error analysis of the RAG generator using ROUGE-L precision, recall, and F1-score. To better understand system behavior beyond aggregate metrics, we extract representative samples from four categories: low precision, low recall, high precision, and high recall cases.


## 🔴 Low Precision Cases (Hallucination / Over-generation)

These cases correspond to outputs with very low precision, indicating that the generated answers contain a large amount of additional or non-aligned content compared to the ground truth.

Key observations:
- Excessive verbosity and explanation beyond the reference
- Inclusion of structured templates (e.g., “Direct Answer”, “Tafsir Insights”)
- Additional theological or contextual commentary not present in the reference
- Correct information is often present but diluted by extra content

Example behavior:
- The model correctly identifies the core concept (e.g., the three steps in Surah 4:34)
- However, it expands the answer significantly, lowering lexical overlap

📌 Insight:
Low precision is primarily caused by **over-generation and lack of conciseness**, rather than factual errors.



## 🔵 Low Recall Cases (Retrieval Failure)

These cases show poor recall, meaning the generated output fails to capture key elements of the ground truth.

Key observations:
- Missing key factual components from the reference
- Retrieval of partially related or incorrect contextual passages
- Occasional misalignment between question and retrieved tafsir
- Short or incomplete answers generated by the model

📌 Insight:
Low recall is mainly due to **retrieval failures or missing relevant context**, indicating limitations in the retrieval or reranking stage.



## 🟢 High Precision Cases (Concise and Aligned Outputs)

These cases exhibit strong precision, meaning most generated content matches the reference.

Key observations:
- Accurate and concise reproduction of key facts
- Minimal hallucination or irrelevant content
- High lexical alignment with ground truth
- Well-structured and focused answers

📌 Insight:
High precision indicates that when retrieval is correct and generation is constrained, the model produces **high-quality and faithful responses**.


## 🟣 High Recall Cases (Good Coverage)

These cases show high recall, meaning most of the reference information is captured in the generated output.

Key observations:
- Strong coverage of ground truth information
- Sometimes includes additional explanations or paraphrasing
- Minor verbosity does not significantly affect correctness
- Good alignment with reference content structure

📌 Insight:
High recall indicates that the system is capable of **capturing most relevant knowledge when retrieval is successful**, even if wording differs.
...
- The system shows **strong variability across samples**
- Failures are mainly due to:
  - Retrieval instability (low recall cases)
  - Over-generation and verbosity (low precision cases)
- When retrieval succeeds, generation quality is generally strong

### Key conclusion:
The main bottleneck in the pipeline is not the generator itself, but the **consistency of retrieval and alignment between retrieved context and generated output**.


### Implication for Improvement

Future improvements should focus on:
- Enhancing retrieval robustness (top-k relevance consistency)
- Improving reranking stability
- Controlling generation verbosity (reducing unnecessary expansions)
- Adding grounding constraints in the prompt

While ROUGE-L provides a useful lexical overlap-based evaluation of the generated responses, it is inherently limited in capturing semantic similarity, especially in cases where the model produces valid paraphrases or more detailed explanations than the reference answers. As observed in our ROUGE analysis, the generator often receives lower precision scores due to its tendency to generate longer, structured, and explanatory outputs that extend beyond the ground-truth text.

To address this limitation, we complement ROUGE with BERTScore, which evaluates semantic similarity using contextual embeddings rather than exact token overlap. The results show significantly stronger and more stable performance, with average scores of approximately 0.82 precision, 0.86 recall, and 0.84 F1. This indicates that although the model diverges lexically from the reference answers, it remains highly aligned at the semantic level.


The evaluation results are presented in the figure below:

![alt text](resources/doc/image_7.png)

Overall, the comparison between ROUGE and BERTScore highlights that the generator is semantically accurate but lexically verbose, confirming that ROUGE underestimates the true quality of the system while BERTScore provides a more faithful reflection of its performance.
